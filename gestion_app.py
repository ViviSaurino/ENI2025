...
def check_app_password() -> bool:
    ...
    <style>
      .eni-hero-wrapper{
        max-width: 1200px;
        margin: 0 auto;          /* centra todo el bloque izquierda + muñecos */
      }
      .eni-hero-title{
        font-size:96px;
        font-weight:800;
      ...
      /* Mismo ancho para píldora + input + botón (aprox. ancho de VENIDOS) */
      .eni-form-wrap{
        max-width:400px;
      }
    ...
    with col1:
        ...
        st.markdown("</div>", unsafe_allow_html=True)  # cierre .eni-form-wrap
    ...
    with col2:
        ...
        if hero_video.exists():
            ...
            # Pegado hacia la izquierda para juntarlo más al texto
            video_html = f"""
