# gestion_app.py — Login + Gestión de tareas en una sola página
...
# Oculta navegación nativa de páginas en el sidebar (para que no aparezca “Páginas”)
st.markdown("""
<style>
...
# ================== RESOLVER RUTA DE KANBAN (sigue en /pages) ==================
def _norm(s: str) -> str:
...
KB_PAGE = _resolve_kanban()  # p.ej. "pages/03_kanban.py"

# ================== LOGIN (en esta misma página) ==================
...
user = google_login(
    ...
    redirect_page=None,   # no saltamos a otra página: la UI real vive aquí
)
if not user:
    st.stop()  # se está mostrando el login (auth_google dibuja el botón)

# ================== SIDEBAR (ESTA PÁGINA = Gestión; Kanban va como link) ==================
with st.sidebar:
    st.header("Secciones")
    # Esta página ES "Gestión de tareas": la marcamos como texto activo
    st.markdown("**📁 Gestión de tareas**")
    if KB_PAGE:
        st.page_link(KB_PAGE, label="Kanban", icon="🧩")
    st.divider()
    sidebar_userbox(user)  # muestra nombre/correo y botón Cerrar sesión

# ================== CONTENIDO PRINCIPAL: GESTIÓN DE TAREAS ==================
st.title("🗂️ Gestión de tareas")

def render_gestion_tareas():
    """
    # ============================
...
# ❌ set_page_config NO aquí (lo pone la página wrapper)

# ---- Auth utils (con fallback seguro si el módulo no está disponible) ----
try:
    from auth_google import google_login, logout
except Exception:
    def google_login():
        st.warning("⚠️ No se encontró 'auth_google'. Continuando sin login real (modo desarrollo).")
...
# --- Sidebar: navegación FIJA y correcta ---
with st.sidebar:
    st.header("Secciones")
...
# =========================
# 🚧 GATE DE AUTENTICACIÓN (con UI mínima en vez de st.stop() directo)
# =========================
def _is_authed() -> bool:
    ss = st.session_state
    return bool(
...
if not _is_authed():
    ...
    st.info("🔐 Por favor, inicia sesión para ver esta sección.")
    if st.button("Iniciar sesión con Google"):
        google_login()
    st.stop()

# ====== ⬇️⬇️ CSS de espaciado entre título, píldoras y secciones (corregido) ⬇️⬇️ ======
st.markdown("""
<style>
/* Espacio debajo del H1 principal */
.block-container h1 { margin-bottom: 25px !important; }

/* Barras superiores con las píldoras */
.topbar, .topbar-ux, .topbar-na { margin-top: 12px !important; margin-bottom: 12px !important; }

/* Evitar márgenes extra del botón */
.topbar .stButton, .topbar-ux .stButton, .topbar-na .stButton { margin: 0 !important; }

/* Tira de ayuda */
.help-strip { margin-top: 8px !important; margin-bottom: 12px !important; }

/* Tarjetas/secciones */
.form-card { margin-top: 19px !important; margin-bottom: 28px !important; padding-top: 12px !important; }
</style>
""", unsafe_allow_html=True)
...
def render():
    ...
    """
    # El cuerpo está en la segunda parte del archivo (que aún no pegaste).
...
if "df_main" not in st.session_state:
    ...
    # Evita KeyError si hay columnas adicionales: intersecta columnas
...
/* Sidebar */
[data-testid="stSidebar"]{ background:var(--lilac-50) !important; border-right:1px solid #ECE6FF !important; transition: width .2s ease, min-width .2s ease; }
...
.form-title,.form-title-ux,.form-title-na{
  ...
  box-shadow:0 6px 16px rgba(148,190,234,.3) !important; min-height:var(--pill-h) !important; height:var(--pill-h) !important; line-height:1 !important; font-size:14px !important;
}
/* SELECTs (corregido !important) */
...
.topbar .stButton, .topbar-ux .stButton, .topbar-na .stButton{ display:inline-flex !important; align-items:center !important; }
.topbar .stButton>button, .topbar-ux .stButton>button, .topbar-na .stButton>button, .pill-btn .stButton>button{
...
.form-title-pri, .form-title-eval{
  ...
  justify-content:center !important; box-shadow:0 6px 16px rgba(214,154,194,.30) !important; min-height:var(--pill-h) !important; height:var(--pill-h) !important; line-height:1 !important;
}
/* Compactación general */
.block-container [data-testid="stVerticalBlock"]{ row-gap:10px !important; gap:6px !important; }
...
.block-container [data-testid="stVerticalBlock"]:has(.topbar-eval){ row-gap:4px !important; gap:4px !important; }
/* Alineación robusta botón + píldora */
.topbar, .topbar-ux, .topbar-na, .topbar-pri, .topbar-eval{ align-items:center !important; }
...
.block-container .stMarkdown:has(.form-title-eval){ margin:0 !important; }
/* ULTRA PATCH: eliminar “cuadradito” del toggle SOLO en #ntbar */
#ntbar .toggle-icon, #ntbar .toggle-icon .element-container, #ntbar .toggle-icon [data-testid="stVerticalBlock"],
#ntbar .toggle-icon [data-testid="stHorizontalBlock"], #ntbar .toggle-icon [data-testid="stButton"]{
  background:transparent !important; border:0 !important; box-shadow:none !important; padding:0 !important; margin:0 !important;
}
#ntbar .toggle-icon .stButton, #ntbar .toggle-icon .stButton > div, #ntbar .toggle-icon [data-testid^="stButton"],
#ntbar .toggle-icon [data-testid^="stButton"] > div, #ntbar .toggle-icon [data-testid^="baseButton"],
#ntbar .toggle-icon [data-testid^="baseButton"] > div, #ntbar .toggle-icon [data-testid^="baseButton"] > div > div{
  background:transparent !important; border:0 !important; box-shadow:none !important; padding:0 !important; margin:0 !important; min-height:0 !important;
}
#ntbar .toggle-icon :is(button, [role="button"], [data-testid^="baseButton"] button){
  all:unset !important; display:inline !important; cursor:pointer !important; user-select:none !important; font-weight:800 !important; font-size:20px !important; line-height:1 !important; color:inherit !important; transform: translateY(8px);
}
#prior-grid .ag-header-viewport, #prior-grid .ag-center-cols-viewport,
#eval-grid  .ag-header-viewport, #eval-grid  .ag-center-cols-viewport{ padding-left:0 !important; padding-right:0 !important; }
#prior-grid .ag-header, #prior-grid .ag-center-cols-container,
#eval-grid  .ag-header, #eval-grid  .ag-center-cols-container{ transform: translateX(0) !important; }
#prior-grid .ag-header-cell, #prior-grid .ag-cell, #eval-grid  .ag-header-cell, #eval-grid  .ag-cell{ border-right:1px solid #E9EDF3 !important; }
</style>
""", unsafe_allow_html=True)
...
import re
import pandas as pd
from datetime import datetime as _dt_for_ids
...
# ====== Callback opcional: hora automática al elegir fecha (fuera de forms) ======
...
st.markdown('</div>', unsafe_allow_html=True)
    ...
    # ---- Render mínimo para no romper mientras pegas tu UI real ----
    init_data()  # asegura st.session_state["df_main"]
    df = st.session_state["df_main"]

    with st.expander("Ver datos cargados (temporal mientras pegas tu UI)", expanded=False):
        st.dataframe(df, use_container_width=True)

    # Ejemplo de pequeño filtro (puedes borrar cuando pegues tu UI)
    c1, c2 = st.columns([1,1])
    areas = ["Todas"] + sorted([x for x in df.get("Área", pd.Series(dtype=str)).astype(str).unique() if x and x != "nan"])
    area_f = c1.selectbox("Filtrar por área (demo)", areas, index=0)
    resps = ["Todos"] + sorted([x for x in df.get("Responsable", pd.Series(dtype=str)).astype(str).unique() if x and x != "nan"])
    resp_f = c2.selectbox("Filtrar por responsable (demo)", resps, index=0)

    dfv = df.copy()
    if "Área" in dfv.columns and area_f != "Todas":
        dfv = dfv[dfv["Área"].astype(str) == area_f]
    if "Responsable" in dfv.columns and resp_f != "Todos":
        dfv = dfv[dfv["Responsable"].astype(str) == resp_f]

    st.markdown("### Vista rápida (demo)")
    st.dataframe(dfv, use_container_width=True)
    # ---------------------------------------------------------------

# Llama a tu vista (cuando pegues tu UI, esta función será tu Gestión completa)
render_gestion_tareas()
