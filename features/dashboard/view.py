...
# Pestaña de Google Sheets a usar (coincide con dashboard)
SHEET_TAB = "TareasRecientes"
...
# ---------- WRITE helper (antes faltaba y causaba el error) ----------
def _write_sheet_tab(df: pd.DataFrame) -> tuple[bool, str]:
    """
    Sube/actualiza a Google Sheets (pestaña SHEET_TAB) haciendo upsert por 'Id'.
    Asegura 'UserEmail' para trazabilidad.
    """
    if not (open_sheet_by_url and upsert_by_id):
        return False, "Faltan utilidades de Google Sheets (utils/gsheets.py)."
    try:
        ...
        res = upsert_by_id(sh, SHEET_TAB, out, id_col="Id")
        if res.get("ok"):
            return True, res.get("msg", "Datos enviados a Sheets (upsert por Id).")
        return False, res.get("msg", "No se pudo escribir en Sheets.")
    except KeyError:
        return False, "Falta `gsheets_doc_url` o credenciales en st.secrets."
...
def render(user: dict | None = None):
    ...
    .ag-theme-balham .row-deleted .ag-cell {
      ...
      color: #7F1D1D !important; opacity: 0.95;
    }
    ...
    # === Edición SOLO en: Tarea, Tipo, Detalle ===
    ...
    if isinstance(grid, dict) and "data" in grid and grid["data"] is not None:
        try:
            ...
            # Merge seguro (mantiene todo; aplica cambios del grid)
    ...
    # ---- Botones alineados EXACTAMENTE bajo filtros ----
    left_spacer = A_f + Fw_f + T_width_f  # ocupa Área + Fase + Responsable

    st.markdown('<div class="hist-actions">', unsafe_allow_html=True)
    # 4 botones iguales (mismo ancho)
    _spacer, b_xlsx, b_save_local, b_save_sheets, b_pull = st.columns([left_spacer, 1, 1, 1, 1], gap="medium")
    ...
    with b_xlsx:
        try:
            ...
            st.error("No pude generar Excel: falta instalar 'xlsxwriter' u 'openpyxl'.")
        except Exception as e:
    ...
    # 3) Subir a Sheets (upsert por Id) — usa el helper nuevo
    with b_save_sheets:
    ...
    # 4) Sincronizar desde Sheet (admin) — en la MISMA FILA
    with b_pull:
        email = st.session_state.get("user_email") or (st.session_state.get("user") or {}).get("email", "")
        sync_admins = set(st.secrets.get("sync", {}).get("admins", []))
        IS_SYNC_ADMIN = (email in sync_admins) or bool(st.session_state.get("acl_user", {}).get("can_sync", False))

        if IS_SYNC_ADMIN:
            if st.button("🔄 Sincronizar desde Sheet (admin)", use_container_width=True, key="btn_pull_sheet_hist"):
                try:
                    if not (open_sheet_by_url and read_df_from_worksheet):
                        st.warning("Faltan utilidades de Google Sheets (utils/gsheets.py).")
                    else:
                        url = st.secrets["gsheets_doc_url"]
                        sh = open_sheet_by_url(url)
                        df_sheet = read_df_from_worksheet(sh, SHEET_TAB)
                        st.session_state["df_main"] = df_sheet.copy()
                        st.success("Sincronizado desde Google Sheets (reemplazo total).")
                        st.rerun()
                except Exception as e:
                    st.warning(f"No se pudo sincronizar desde Sheets: {e}")
        else:
