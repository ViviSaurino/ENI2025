...
def _to_naive_local_one(x):
    ...
    try:
        if isinstance(x, pd.Timestamp):
            if x.tz is not None:
                d = (
                    x.tz_convert(_TZ or x.tz).tz_localize(None)
                    if _TZ
                    else x.tz_localize(None)
                )
                return d
            return x
        s = str(x).strip()
...
# --- Upsert a Sheets usando helper centralizado (si existe) ---
try:
...
def _sheet_upsert_estado_by_id(df_base: pd.DataFrame, changed_ids: list[str]):
    """Empuja SOLO las columnas de estado/fechas para los Id cambiados."""
...
def render(user: dict | None = None):
    # Si en algún sitio controlas visibilidad con esta clave, respétala;
    # si no existe, asumimos que la vista es visible.
    if not st.session_state.get("est_visible", True):
        return

    # Para el ancho del botón guardar (igual que antes)
...
<style>
  ...
  /* Tarjetas debajo del encabezado */
  ...
  /* Botón Volver con mismo look "botón" */
  ...
  /* Estado actual neutro (sin color fuerte en header) */
...
""",
    ...
    if not is_super:
        ...
        if isinstance(df_all, pd.DataFrame) and "Responsable" in df_all.columns and me:
            try:
                mask = df_all["Responsable"].astype(str).str.contains(
        ...
        if (
            ...
            and "Responsable" in df_all.columns
            and me
        ):
            try:
                mask = df_all["Responsable"].astype(str).str.contains(
    ...
    # Rango por defecto
    fr_all = pd.to_datetime(
        df_all.get("Fecha Registro", pd.Series([], dtype=object)), errors="coerce"
    )
    if fr_all.notna().any():
    ...
    if is_super:
        ...
        est_resp = r1_c1.selectbox(
            "Responsable", ["Todos"] + resp_all, index=0, key="est_resp_v4"
        )
        ...
        est_fase = r1_c2.selectbox(
            "Fase", ["Todas"] + fases_all, index=0, key="est_fase_v4"
        )
        ...
        est_tipo = r1_c3.selectbox(
            "Tipo de tarea", ["Todos"] + tipos_all, index=0, key="est_tipo_v4"
        )

        estado_opts_labels = ["Todos"] + [estado_labels[e] for e in estados_catalogo]
        sel_label = r1_c4.selectbox(
            "Estado actual", estado_opts_labels, index=0, key="est_estado_v4"
        )
        est_estado = (
            "Todos"
            if sel_label == "Todos"
        ...
        est_fase = r1_c1.selectbox(
            "Fase", ["Todas"] + fases_all, index=0, key="est_fase_v4_nosuper"
        )
        ...
        est_tipo = r1_c2.selectbox(
            "Tipo de tarea",
            ["Todos"] + tipos_all,
            index=0,
            key="est_tipo_v4_nosuper",
        )

        estado_opts_labels = ["Todos"] + [estado_labels[e] for e in estados_catalogo]
        sel_label = r1_c3.selectbox(
            "Estado actual",
            estado_opts_labels,
            index=0,
            key="est_estado_v4_nosuper",
        )
        est_estado = (
            "Todos"
            if sel_label == "Todos"
    ...
    # Línea inferior y fila de botones (dentro de la "zona roja")
    st.markdown('<div class="est-filter-line"></div>', unsafe_allow_html=True)
    ...
    fi_eff = pd.to_datetime(
        df_tasks.get("Fecha de inicio", pd.Series([], dtype=object)), errors="coerce"
    )
    ft_eff = pd.to_datetime(
        df_tasks.get("Fecha terminada", pd.Series([], dtype=object)), errors="coerce"
    )
    fe_eff = pd.to_datetime(
        df_tasks.get("Fecha eliminada", pd.Series([], dtype=object)), errors="coerce"
    )
    estado_calc = pd.Series("No iniciado", index=df_tasks.index, dtype="object")
    estado_calc = estado_calc.mask(
        fi_eff.notna() & ft_eff.isna() & fe_eff.isna(), "En curso"
    )
    ...
    # Siempre filtramos según los valores actuales
    ...
    if est_estado != "Todos":
        df_tasks = df_tasks[
            df_tasks["_ESTADO_EFECTIVO_"].astype(str) == est_estado
        ]

    fcol = pd.to_datetime(
        df_tasks.get("Fecha Registro", pd.Series([], dtype=object)), errors="coerce"
    )
    if est_desde:
    ...
    if not df_tasks.empty:
        ...
        if "Estado" in base.columns:
            ...
            est_now = estado_guardado.where(
                ~estado_guardado.isin(["", "nan", "NaN", "None"]), est_now
            )
    ...
    with u2:
        if st.button("💾 Guardar", use_container_width=True, key="est_guardar_inline_v4"):
            try:
                ...
                # --- Autogenerar Ids vacíos en la vista
                id_series = grid_data["Id"].astype(str).str.strip()
                mask_blank_ids = id_series.str.lower().isin(
                    {"", "-", "nan", "none", "null"}
                )
                ...
                fi_new_vis = norm(
                    g_i.get("Fecha de inicio", pd.Series(index=g_i.index))
                )
                hi_new = norm(g_i.get("Hora de inicio", pd.Series(index=g_i.index)))
                ft_new_vis = norm(
                    g_i.get("Fecha terminada", pd.Series(index=g_i.index))
                )
                ht_new = norm(g_i.get("Hora terminada", pd.Series(index=g_i.index)))
                fe_new_vis = norm(
                    g_i.get("Fecha eliminada", pd.Series(index=g_i.index))
                )
                he_new = norm(g_i.get("Hora eliminada", pd.Series(index=g_i.index)))
                fc_new_vis = norm(
                    g_i.get("Fecha cancelada", pd.Series(index=g_i.index))
                )
                hc_new = norm(g_i.get("Hora cancelada", pd.Series(index=g_i.index)))
                fp_new_vis = norm(
                    g_i.get("Fecha pausada", pd.Series(index=g_i.index))
                )
                ...
                me = _display_name().strip()
                is_super_local = _is_super_editor()
                if not is_super_local and "Responsable" in base.columns and me:
                    mask_me = base["Responsable"].astype(str).str.contains(
                        me, case=False, na=False
                    )
                    base = base[mask_me]
                ...
                if mask_blank_ids.any():
                    tarea_by_newid = {
                        row["Id"]: row.get("Tarea", "") for _, row in grid_data.iterrows()
                    }
                    base_blank = base["Id"].astype(str).str.strip().isin(
                        {"", "-", "nan", "none", "null"}
                    )
                    if base_blank.any():
                        for idx_base in base.index[base_blank]:
                            ...
                            for nid, t_grid in tarea_by_newid.items():
                                if not nid:
                                    continue
                                if str(t_grid or "").strip() and t_base == str(
                                    t_grid
                                ).strip():
                ...
                if not is_super_local:
                    cur_start = (
                        base.groupby("Id", as_index=True)["Fecha inicio"]
                        .last()
                        .map(_canon_str)
                    )
                    cur_start_alias = (
                        base.groupby("Id", as_index=True)["Fecha de inicio"]
                        .last()
                        .map(_canon_str)
                    )
                    ...
                    bad_term = [
                        i
                        for i in ids_ok
                        if (_canon_str(ft_new_vis.get(i, "")) and not _has_start(i))
                    ]
                    if bad_term:
                        st.warning(
                            "No puedes registrar 'Fecha terminada' sin 'Fecha de inicio' en algunas tareas."
                        )
                        for i in bad_term:
                            ft_new_vis[i] = ""
                            ht_new[i] = ""
                    ...
                    cur_end = (
                        base.groupby("Id", as_index=True)["Fecha terminada"]
                        .last()
                        .map(_canon_str)
                    )
                    cur_end_alias = (
                        base.groupby("Id", as_index=True)["Fecha Terminado"]
                        .last()
                        .map(_canon_str)
                    )
                    ...
                    bad_del = [
                        i
                        for i in ids_ok
                        if (_canon_str(fe_new_vis.get(i, "")) and not _has_end(i))
                    ]
                    if bad_del:
                        st.warning(
                            "No puedes registrar 'Fecha eliminada' sin 'Fecha terminada' en algunas tareas."
                        )
                        for i in bad_del:
                            fe_new_vis[i] = ""
                            he_new[i] = ""
                ...
                me_alias = (
                    resp_alias.get(me_email) or _display_name().strip() or me_email
                )

                if not is_super_local:
                    for i in ids_ok:
                        prev_owner = (
                            _canon_str(base_idx.at[i, "OwnerEmail"])
                            if i in base_idx.index
                            else ""
                        )
                        prev_resp = (
                            _canon_str(base_idx.at[i, "Responsable"])
                            if i in base_idx.index
                            else ""
                        )
                        changed_local = False
                ...
                for i in ids_ok:
                    ...
                    prev_fiA = (
                        _canon_str(base_idx.at[i, "Fecha de inicio"])
                        if i in base_idx.index
                        else ""
                    )
                    prev_hi = (
                        _canon_str(base_idx.at[i, "Hora de inicio"])
                        if i in base_idx.index
                        else ""
                    )
                    ...
                    prev_ft = (
                        _canon_str(base_idx.at[i, "Fecha Terminado"])
                        if i in base_idx.index
                        else ""
                    )
                    prev_ftA = (
                    ...
                    prev_ht = (
                        _canon_str(base_idx.at[i, "Hora Terminado"])
                        if i in base_idx.index
                        else ""
                    )
                    ...
                    prev_fe = (
                        _canon_str(base_idx.at[i, "Fecha eliminada"])
                        if i in base_idx.index
                        else ""
                    )
                    prev_he = (
                        _canon_str(base_idx.at[i, "Hora eliminada"])
                        if i in base_idx.index
                        else ""
                    )
                    ...
                    prev_fc = (
                        _canon_str(base_idx.at[i, "Fecha cancelada"])
                        if i in base_idx.index
                        else ""
                    )
                    prev_hc = (
                        _canon_str(base_idx.at[i, "Hora cancelada"])
                        if i in base_idx.index
                        else ""
                    )
                    ...
                    prev_fp = (
                        _canon_str(base_idx.at[i, "Fecha pausada"])
                        if i in base_idx.index
                        else ""
                    )
                    prev_hp = (
                        _canon_str(base_idx.at[i, "Hora pausada"])
                        if i in base_idx.index
                        else ""
                    )
                    ...
                    for i in changed_ids:
                        if i in base_idx.index:
                            for col in cols_apply:
                                ...
                                full_updated.loc[
                                    full_updated["Id"].astype(str) == i, col
                                ] = val
                ...
                try:
                    if DO_SHEETS_UPSERT and changed_ids:
                        _sheet_upsert_estado_by_id(
                            full_updated.copy(), sorted(changed_ids)
                        )
    ...
    # Cerrar divs HTML
    st.markdown("</div>", unsafe_allow_html=True)  # section-est
    st.markdown("</div>", unsafe_allow_html=True)  # est-section
