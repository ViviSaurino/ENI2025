...
def _allowed_editors_from_secrets() -> set[str]:
    ...
    Lee lista de correos permitidos desde secrets:
      ...
      - editors.emails
      - editor_emails
    Fallback: PRIORITY_EDITORS (env) y lista fija (Vivi/Enrique).
    """
    allow: set[str] = set()
    try:
        allow = {
...
def _is_super_priority_editor(user: dict | None = None) -> bool:
    """Super-editor: Vivi / Enrique / can_edit_all / can_edit_all_tabs / en lista de editores."""
    acl_user = st.session_state.get("acl_user", {}) or {}
    if str(acl_user.get("can_edit_all", "")).strip().lower() in {"1","true","yes","si","sí"}:
        return True
    if bool(acl_user.get("can_edit_all_tabs", False)):
        return True
    dn = (_get_display_name() or "").strip().lower()
...
def _norm_pri(txt: str) -> str:
    """Canónico sin emoji: Sin asignar, Urgente, Alto, Medio, Baja."""
    t = (_strip_emoji(txt) or "").strip().lower()
    if t in {"", "sin asignar", "sin asignar prioridad", "sin prioridad", "ninguna", "none","nan","na","null","-","—"}:
        return "Sin asignar"
    if t == "urgente":
        return "Urgente"
    if t in {"alto", "alta"}:
...
def _display_with_emoji(label: str) -> str:
    """Agrega emoji a la etiqueta (excepto 'Sin asignar')."""
...
# ====== Carga SOLO desde Google Sheets ======
def _get_sheet_conf():
    ss_url = (
...
def render(user: dict | None = None):
    ...
    if st.session_state["pri_visible"]:
        ...
        <style>
          ...
          #pri-section .ag-theme-balham .ag-header{ height: 56px !important; min-height: 56px !important; }
          ...
          #pri-section .ag-theme-balham .ag-header-cell-text{ font-weight: 500 !important; }

          :root{
            --pri-pill: #A7F3D0;  --pri-help-bg: #ECFDF5; --pri-help-border: #A7F3D0; --pri-help-text: #047857;
          }
          .pri-pill{
          ...
          .help-strip-pri{
            background: var(--pri-help-bg); border: 1px solid var(--pri-help-border);
            color: var(--pri-help-text); border-radius: 10px; padding: 8px 12px; margin: 8px 0 12px 0;
          }
        </style>
        """,
        ...
        # ===== Estado actual calculado (para filtros) =====
        fi = pd.to_datetime(df_all.get("Fecha de inicio", df_all.get("Fecha inicio", pd.Series([], dtype=object))), errors="coerce")
        ft = pd.to_datetime(df_all.get("Fecha terminada", df_all.get("Fecha Terminado", pd.Series([], dtype=object))), errors="coerce")
        ...
        with st.form("pri_filtros_v2", clear_on_submit=False):
            if IS_SUPER_VIEWER:
                c_resp, c_fase, c_tipo, c_estado, c_desde, c_hasta, c_buscar = st.columns([Fw, Fw, T_width, D, D, R, C], gap="medium")
            else:
                c_fase, c_tipo, c_estado, c_desde, c_hasta, c_buscar = st.columns([Fw, T_width, D, D, R, C], gap="medium")
                c_resp = None
            ...
            pri_estado = "Todos" if sel_label == "Todos" else [k for k, v in estado_labels.items() if v == sel_label][0]
            ...
            if IS_SUPER_VIEWER:
                ...
                if pri_tipo != "Todos" and "Tipo de tarea" in df_resp_src.columns:
                    df_resp_src = df_resp_src[df_resp_src["Tipo de tarea"].astype(str) == pri_tipo]
                responsables_all = sorted([x for x in df_resp_src.get("Responsable", pd.Series([], dtype=str)).astype(str).unique() if x and x != "nan"])
                pri_resp = c_resp.selectbox("Responsable", ["Todos"] + responsables_all, index=0)
            else:
                pri_resp = "Todos"
        ...
        if pri_do_buscar:
            ...
            if pri_tipo != "Todos" and "Tipo de tarea" in df_filtrado.columns:
                df_filtrado = df_filtrado[df_filtrado["Tipo de tarea"].astype(str) == pri_tipo]
            if pri_estado != "Todos" and "_ESTADO_PRI_" in df_filtrado.columns:
                df_filtrado = df_filtrado[df_filtrado["_ESTADO_PRI_"].astype(str) == pri_estado]
            ...
            if "Fecha inicio" in df_filtrado.columns:
                ...
                fcol = pd.to_datetime(df_filtrado.get("Fecha", pd.Series([], dtype=str)), errors="coerce")

            if pri_desde is not None:
                df_filtrado = df_filtrado[fcol.isna() | (fcol >= pd.to_datetime(pri_desde))]
            if pri_hasta is not None:
                limite = pd.to_datetime(pri_hasta) + pd.Timedelta(days=1) - pd.Timedelta(seconds=1)
                df_filtrado = df_filtrado[fcol.isna() | (fcol <= limite)]

        # ---- Vista con DOS columnas de prioridad ----
        ...
        # Por pedido: mostrar "Alto" por defecto si está sin asignar (solo en la vista)
        cur_norm_display = cur_norm.replace("Sin asignar", "Alto")
        cur_disp_actual = cur_norm_display.map(_display_with_emoji)

        view = pd.DataFrame({
            "Id": base["Id"].astype(str),
            "Responsable": base["Responsable"].astype(str),
            "Fase": base["Fase"].astype(str),
            "Tipo de tarea": base["Tipo de tarea"].astype(str).replace({"nan": "—", "None": "—", "": "—"}),
            "Tarea": base["Tarea"].astype(str),
            "Prioridad actual": cur_disp_actual,        # solo lectura
            "Prioridad a modificar": [""] * len(base),  # editable
        })

        # baseline canónico REAL (sin el “Alto por defecto” de la vista)
        st.session_state["_pri_base_norm"] = dict(zip(view["Id"].astype(str), cur_norm.astype(str)))

        # ===== Estilo de celdas =====
        priority_cell_style = JsCode(
            """
        function(p){
          const base = {display:'flex', alignItems:'center', justifyContent:'flex-start',
          ...
          if(clean === 'sin asignar' || clean === 'sin asignar prioridad'){
            return Object.assign({}, base, {backgroundColor:'#E0F2FE', color:'#075985', border:'1px solid #BAE6FD'});
          }
          if(clean === 'urgente'){ return Object.assign({}, base, {backgroundColor:'#FEE2E2', color:'#B91C1C', border:'1px solid #FCA5A5'}); }
          if(clean === 'alto' || clean === 'alta'){ return Object.assign({}, base, {backgroundColor:'#FFEAD5', color:'#9A3412', border:'1px solid #FED7AA'}); }
          if(clean === 'medio' || clean === 'media'){ return Object.assign({}, base, {backgroundColor:'#FEF3C7', color:'#92400E', border:'1px solid #FDE68A'}); }
          if(clean === 'bajo' || clean === 'baja'){ return Object.assign({}, base, {backgroundColor:'#DCFCE7', color:'#166534', border:'1px solid #BBF7D0'}); }
          return base;
        }"""
        )

        # ===== GRID =====
        IS_EDITOR_FLAG = IS_EDITOR
        if IS_EDITOR_FLAG:
            col_defs = [
                {"field": "Id", "headerName": "Id", "editable": False, "minWidth": 80, "flex": 0},
                {"field": "Responsable", "headerName": "Responsable", "editable": False, "minWidth": 180, "flex": 1},
                {"field": "Fase", "headerName": "Fase", "editable": False, "minWidth": 130, "flex": 1},
                {"field": "Tipo de tarea", "headerName": "Tipo de tarea", "editable": False, "minWidth": 180, "flex": 1},
                {"field": "Tarea", "headerName": "📝 Tarea", "editable": False, "minWidth": 260, "flex": 2},
                {"field": "Prioridad actual", "headerName": "🏷️ Prioridad actual", "editable": False, "minWidth": 170, "flex": 1, "cellStyle": priority_cell_style},
                {"field": "Prioridad a modificar", "headerName": "🏷️ Prioridad a modificar", "editable": True, "minWidth": 190, "flex": 1,
                 "cellEditor": "agSelectCellEditor", "cellEditorParams": {"values": CHOICES_EDIT_EMO}, "cellStyle": priority_cell_style},
            ]
        else:
            col_defs = [
                {"field": "Id", "headerName": "Id", "editable": False, "minWidth": 80, "flex": 0},
        ...
        grid_options = {
            "columnDefs": col_defs,
            "defaultColDef": {"sortable": False, "filter": False, "floatingFilter": False, "wrapText": False, "autoHeight": False, "resizable": True},
        ...
        grid_resp = AgGrid(
            ...
            update_mode=(GridUpdateMode.MODEL_CHANGED | GridUpdateMode.FILTERING_CHANGED | GridUpdateMode.SORTING_CHANGED | GridUpdateMode.VALUE_CHANGED),
            allow_unsafe_jscode=True,
        )

        # ===== Detectar cambios (comparar canónico REAL vs 'Prioridad a modificar') =====
        changed_ids: list[str] = []
        edits = grid_resp.get("data", [])
        new_df = (pd.DataFrame(edits) if isinstance(edits, list) else pd.DataFrame(grid_resp.data)).copy()

        if IS_EDITOR_FLAG and not new_df.empty:
            base_map = st.session_state.get("_pri_base_norm", {}) or {}  # canónico real
            new_df["Id"] = new_df["Id"].astype(str)
            new_df["__nuevo_norm"] = new_df.get("Prioridad a modificar", "").astype(str).map(_norm_pri)

            def _is_changed(row):
                rid = row["Id"]
                nuevo = row["__nuevo_norm"]
                prev = _norm_pri(base_map.get(rid, ""))
                return (nuevo not in {"", "Sin asignar"}) and (nuevo.lower() != prev.lower())

            mask_changed = new_df.apply(_is_changed, axis=1)
            ...
            if changed_ids:
                ...
                if "Id" in base_full.columns:
                    ...
                    st.session_state["df_main"] = base_full  # Solo memoria; persistimos más abajo en Sheets

        st.session_state["_pri_changed_ids"] = changed_ids

        # ===== BOTÓN GUARDAR (solo Vivi/Enrique) — persiste SOLO en Google Sheets =====
        ...
        if SHOW_BUTTON:
            ...
            with b_action:
                click = st.button("🏷️ Dar prioridad", use_container_width=True, disabled=not IS_EDITOR_FLAG, key="btn_dar_prioridad")

            if click and IS_EDITOR_FLAG:
                try:
                    ...
                    if not ids:
                        ...
                        if upsert_rows_by_id is None:
                            st.warning("No se encontró utils.gsheets.upsert_rows_by_id. Configura utils/gsheets para persistir en Sheets.")
                        else:
                            ss_url, ws_name = _get_sheet_conf()
                            base_full = st.session_state.get("df_main", pd.DataFrame()).copy()
